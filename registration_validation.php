...
include_once("./login/session_control.php");
...
if (!empty($_GET['key'])) {
	...
	try {
		...
		$stmt = $db->query('SELECT COUNT(*) FROM '.$dbTableUsers.' WHERE emailValidationKey="'.$securedEmailActivationKey.'"');
		$validationKeyExist = $stmt->fetchColumn();
		$stmt->closeCursor();
	} catch (Exception $e) {
		viewCatchedError($e->getMessage(), $e->getCode(), $e->getLine(), $e->getFile());
	}
	
	// Test si la clef existe
	if ($validationKeyExist != 0) {
		echo "activation prise en compte";
	} else {
		echo "";
	}
